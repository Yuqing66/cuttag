...
def run(args):
	...
	# fragment
	outFile = os.path.join(odir, inFileBase.replace('.txt','Dedup.txt'))
	out = open(outFile,'w')
	# antibody barcodes frequency within each dups
	freqFile = os.path.join(odir, inFileBase.replace('.txt','_freq.txt'))
	freq = open(freqFile, 'w')
	# cutsite tracks for antibody, barcodes as file names.
	abFiles = {}
	abOuts = {}
	ab_base = inFileBase.replace('.txt','_')
	for key in abdict.keys():
		abFiles[key] = os.path.join(odir, f"{ab_base}{key}.txt")
		abOuts[key] = open(abFiles[key], 'w')
