...
process extractValidReads {
    ...
    container params.extractValidReads_container ?: ''

    input:
    tuple val(sample_id), val(meta), val(chunk_id), path(r1), path(i), path(r2)
    ...
    : \${params.extract_script:?"Set --extract_script to the path of extractValidReads_multiCUTTag_nobccorrect_nextflow.py"}
    : \${params.cell_barcode_reference:?"Set --cell_barcode_reference to the 10x whitelist"}
    ...
    python ${params.extract_script} \
        ...
        -b1 ${b1_list[@]} \
        -b2 ${b2_list[@]} \
        -b ${params.cell_barcode_reference} \
